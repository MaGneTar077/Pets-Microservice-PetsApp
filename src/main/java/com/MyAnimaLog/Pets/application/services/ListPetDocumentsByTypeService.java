...
public class ListPetDocumentsByTypeService implements ListPetDocumentsByTypeUseCase {
    ...
    @Override
    public List<PetDocumentSummaryResponse> execute(UUID petId, DocumentType documentType) {
        petRepository.findById(petId)
                .orElseThrow(() -> new PetNotFoundException(
                        "Pet not found with id: " + petId));

        return petDocumentRepository.findAllByPetIdAndDocumentType(petId, documentType)
                .stream()
                .map(this::toResponse)
                .toList();
    }
